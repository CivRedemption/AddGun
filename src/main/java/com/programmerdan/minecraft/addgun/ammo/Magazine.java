...
public class Magazine {
	...
	private List<Component> exampleLore;
	
	private final Set<String> allowedBullets = Sets.newConcurrentHashSet();
	
	private final Map<String, Integer> allowsRounds = new ConcurrentHashMap<>();
		...
		if (this.example == null) {
			...
			if (this.example.hasItemMeta()) {
				if (this.example.getItemMeta().hasLore()) {
					this.exampleLore = this.example.getItemMeta().lore();
				}
	...
	public ItemStack updateMagazineLore(ItemStack magazine) {
		...
		System.out.println(allowsRounds);
		System.out.println(this.allowsRounds.get(ammo));
		
		if (ammo != null) { // locked / has a bullet
			lore.add(Component.text(ChatColor.GREEN + "Magazine of " + ChatColor.GRAY + ammo));
			if (rounds <= 0) {
				lore.add(Component.text(ChatColor.RED + "  EMPTY " + ChatColor.DARK_AQUA + "(out of " + ChatColor.WHITE + this.allowsRounds.get(ammo) + ChatColor.DARK_AQUA + " max)"));
			} else if (rounds == 1) {
				lore.add(Component.text(ChatColor.GOLD + "  1 " + ChatColor.BLUE + "Round " + ChatColor.DARK_AQUA + "(out of " + ChatColor.WHITE + this.allowsRounds.get(ammo) + ChatColor.DARK_AQUA + " max)"));
			} else {
				lore.add(Component.text(ChatColor.GREEN + String.format("  %d ", rounds) + ChatColor.BLUE + "Rounds " + ChatColor.DARK_AQUA + "(out of " + ChatColor.WHITE + this.allowsRounds.get(ammo) + ChatColor.DARK_AQUA + " max)"));
			}
		} else {
			lore.add(Component.text(ChatColor.RED + "Magazine Empty"));
			lore.add(Component.text(ChatColor.GREEN + "Accepts bullets: "));
